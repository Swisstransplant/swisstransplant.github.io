...
Research tools facilitate state-of-the-art data analysis at Swisstransplant.

## Basic software

The basic software to perform statistical analysis is based on three components: the R statistical software, the RStudio graphical interface, and the Quarto scientific publishing system.
...
::: {.callout-note}
In Windows RStudio desktop requires administrator rights. However, it is also possible to download a zip bundle from the download page, unpack the files into a folder, for example, `software`, and create a shortcut with `rstudio.exe` on the desktop or the taskbar.
:::

## Swisstransplant R package

![](images/swt_hex.png){height=2in style="float:right; padding:10px"}
